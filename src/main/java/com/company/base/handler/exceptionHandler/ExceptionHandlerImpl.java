...
import com.hei.springcloudtest.handler.LambdaHandler;
...
public class ExceptionHandlerImpl implements ExceptionHandler<LambdaUrlResponseEvent> {
    ...
    public LambdaUrlResponseEvent handle(Throwable throwable) {
        ...
        //adding print stack in case we have no appender or we are running inside SAM local , where need the
        // output to go the stderr.
        throwable.printStackTrace();
        return new LambdaUrlResponseEvent(500, HEADERS.toSingleValueMap(), getErrorJson(INTERNAL_SERVER_ERROR));
    }


    protected String getErrorJson(String message) {
        try {
            return LambdaHandler.getObjectMapper().writeValueAsString(message);
